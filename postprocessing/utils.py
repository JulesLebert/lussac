...
def _compute_ISI(spike_train, bin_size, max_time, sampling_f):
	bin_size_c = round(bin_size * 1e-3 * sampling_f)
	max_time_c = round(max_time * 1e-3 * sampling_f)
	max_time -= max_time % bin_size
	n_bins = int(max_time_c / bin_size_c)

	# bins = np.arange(0, max_time_c+bin_size_c, bin_size_c) * 1e3 / sampling_f
	# ISI = np.zeros([n_bins], dtype=np.uint64)

	diff = np.diff(spike_train)
	ISI, bins = np.histogram(diff, bins=n_bins, range=[0,max_time_c])

	# ISI = _compute_ISI_c(hist, train, len(spike_train), bin_size_c, max_time_c)

	return (ISI, bins)


def _compute_ISI_c(ISI, spike_train, N, bin_size, max_time):
...
def _compute_autocorr(spike_train,  bin_size,  max_time,  sampling_f):
	bin_size_c = round(bin_size * 1e-3 * sampling_f)
	max_time_c = round(max_time * 1e-3 * sampling_f)
	max_time -= max_time % bin_size
	n_bins = int(2 * (max_time_c / bin_size_c))

	bins = np.arange(-max_time_c, max_time_c+bin_size_c, bin_size_c) * 1e3 / sampling_f
	auto_corr = np.zeros([n_bins], dtype=np.uint64)

	# auto_corr = np.correlate(spike_train, spike_train, mode='full')
	# train = spike_train
	# corr = auto_corr

	corr = _compute_autocorr_c(auto_corr, spike_train, len(spike_train), n_bins, bin_size_c, max_time_c)

	return (corr, bins)


def _compute_autocorr_c(auto_corr, spike_train, N, n_bins, bin_size, max_time):
	...
	for i in range(N):
		for j in range(i+1, N):
			...
			bin = int(diff / bin_size)
			auto_corr[int(n_bins/2) - bin - 1] += 1
			auto_corr[int(n_bins/2) + bin] += 1
...
def _compute_crosscorr(spike_train1, spike_train2,  bin_size,  max_time,  sampling_f):
	max_time_c = round(max_time * 1e-3 * sampling_f)
	bin_size_c = round(bin_size * 1e-3 * sampling_f)
	max_time -= max_time % bin_size
	n_bins = int(2 * (max_time_c / bin_size_c))
	...
	bins = np.arange(-max_time_c, max_time_c+bin_size_c, bin_size_c) * 1e3 / sampling_f
	cross_corr = np.zeros([n_bins], dtype=np.uint64)


	corr = _compute_crosscorr_c(cross_corr, spike_train1, spike_train2, len(spike_train1), len(spike_train2), n_bins, bin_size_c, max_time_c)

	return (corr, bins)


def _compute_crosscorr_c(cross_corr, spike_train1, spike_train2,  N1,  N2,  n_bins,  bin_size,  max_time):
...
def _compute_firing_rate(spike_train,  bin_size,  t_max,  as_Hz,  sampling_f):
	bin_size_c = round(bin_size * sampling_f)

	bins = np.arange(0, t_max+bin_size_c, bin_size_c) / sampling_f
	firing_rate = np.zeros([len(bins)-1], dtype=np.uint64)

	hist = _compute_firing_rate_c(firing_rate, spike_train, len(spike_train), len(bins)-1, bin_size_c)

	if as_Hz:
		return (hist / bin_size, bins)
	else:
		return (hist, bins)


def _compute_firing_rate_c(firing_rate, spike_train,  N,  n_bins,  bin_size):
	
	for i in range(N):
		bin = int(spike_train[i] / bin_size)
		if bin >= n_bins:
...
def _compute_nb_violations(spike_train,  t_r,  sampling_f):
	t_r_c = round(t_r * 1e-3 * sampling_f)
	train = spike_train.data

	n_v = _compute_nb_violations_c(train, len(spike_train), t_r_c)

	return n_v

def _compute_nb_violations_c( spike_train,  N,  t_r):
	nb_pairs = 0
	nb_half_pairs = 0
...
def _compute_nb_coincident_spikes(spike_train1, spike_train2, max_time):
	train1 = spike_train1.data
	train2 = spike_train2.data
...
def _compute_nb_coincident_spikes_c(spike_train1, spike_train2,  N1,  N2,  max_time):
	i, j, diff
	nb_pairs = 0
...
def _compute_cross_violations(spike_train1, spike_train2, t_r, sampling_f):
	t_r_c = round(t_r * 1e-3 * sampling_f)
	train1 = spike_train1.data
	train2 = spike_train2.data
...
def _compute_cross_violations_c(spike_train1, spike_train2, N1, N2, t_r):
	i, j, diff
	nb_pairs = 0
	nb_half_pairs = 0
